...
当前实际实现两个转换：`symbol.map.v1` 要求显式的一对一映射，属于无损、可逆转换；`bar.resample.v1` 将更细 bar 聚合为较粗 bar，属于有损、不可逆转换。重采样按市场 `event_time` 决定 open/close 顺序，结果的 `available_time` 和 `receive_time` 取全部必要输入的最晚值；同一标的和桶内重复市场时间被视为无法区分的修订并明确拒绝。二者可以在一份计划中按记录顺序组合。
