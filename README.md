# Paper Strategy Runtime Contract 2.8.3
...
构建阶段使用 `--no-cache --pull` 安装依赖和取得论文；验证阶段为 `--network none`、只读根目录、非 root、无 capabilities、`no-new-privileges`、CPU/内存/PID 限制和 `noexec` 临时目录。验证脚本直接调用镜像内解释器，**不会在断网阶段运行 uv sync、pip 或项目构建**。启动器先使旧成功收据失效，构建、镜像检查或运行失败均写入本轮失败状态；成功时才把同一 `attempt_id` 的镜像、验证和验收结果发布到 `reports/strict/`。
