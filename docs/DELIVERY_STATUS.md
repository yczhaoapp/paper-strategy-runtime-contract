...
已完成当前工作区中的 2.6 契约语义收口，状态为 **S 级候选实现，宿主机和 Linux/arm64 严格容器完整验收通过**。最终认证仍由独立评审决定。

## 实际执行结果

| 检查 | 结果 | 证据 |
| --- | --- | --- |
| 2.6 macOS / Python 3.13.7 完整门禁 | 通过；当前输入树哈希已写入收据 | `evidence/release/host-verification.json` |
| 2.6 pytest | 186 通过，0 失败，0 错误，0 跳过 | `evidence/release/host-acceptance.json`；完整 JUnit 位于本地报告/CI artifact |
| 2.6 核心覆盖率 | 宿主与严格容器均 ≥92.5% | 两份已发布 acceptance 记录精确值；可选 Nautilus 单独排除 |
| 2.6 Linux/arm64 / Python 3.12.14 严格 Docker | 干净上下文构建和断网运行通过；18 个运行包均为 R2 | `evidence/release/strict-verification.json` 与 `strict-acceptance.json` |
| Ruff + 严格 mypy | 通过 | ruff.log / mypy.log |
| 硬验收项 | 18/18 通过 | acceptance-report.json |
...
2.6 已在 Docker Desktop 的 Linux/arm64 VM 内从排除本机 `papers/sources` 缓存的构建上下文重新构建；构建阶段实际获取并校验 17 份论文。随后实测断网、只读根目录、非 root、capabilities 清零、`NoNewPrivs`、PID/CPU/内存限制和 `noexec` 临时目录。严格运行调用镜像构建期安装的解释器，不在断网阶段调用 uv、pip 或项目构建。
