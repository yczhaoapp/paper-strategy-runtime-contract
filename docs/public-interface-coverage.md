...
| 最小回测 | `BacktestAdapter.capabilities`、`BacktestAdapter.run` / `psrc run` | Reference 全矩阵；Backtrader 规则差分及监督/RL 完整生命周期；orchestrator 与直接 Adapter 调用的计划/策略/运行时/引擎/沙箱错配负例 | NautilusTrader 为可选依赖，不能作为默认实测能力 |
