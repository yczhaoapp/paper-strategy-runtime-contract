...
| 监督学习 | L2 成交概率 | θ=0 常数死亡率、500ms 有限期限 Erlang 队列竞争 | Lokin & Yu 2026 | 公式复现 / A2 |
| 监督学习 | 截面排序器 | pooled OLS 预测并截面排序 | Gu, Kelly & Xiu 2019 | 等价方法复现 / A1 |
| 强化学习 | Tabular Q 库存 | 离策略离散库存控制 | Spooner et al. 2018 | 方法适配 / A0 |
| 强化学习 | SARSA 趋势 | 在策略 bar 状态控制 | Spooner et al. 2018 | 方法适配 / A0 |
| 强化学习 | 风险厌恶 Contextual Bandit | 线性后验 Thompson 抽样、均值方差目标 | Lin, Wang & Zhou 2022 | 等价方法复现 / A1 |
| 强化学习 | Double-Q 订单簿库存 | 拆分估计器的 L2 库存控制 | Spooner et al. 2018 | 方法适配 / A0 |
| 强化学习 | A2C 配对 | 固定 1:-2 spread 状态、优势 Actor–Critic 更新 | Yang & Malik 2024 | 方法适配 / A0 |
...
`formula_reproduction` 与 `algorithm_reproduction` 必须达到 A2；`method_reproduction` 对应 A1 等价方法复现；`method_adaptation` 对应 A0。A0 可以计入 18 个可运行策略，但不能计入复现数量。数据与实验结果另行评级；本表的 A2 不代表原论文行情或收益已复现。完整判定见[复现等级政策](reproduction-fidelity-policy.md)。
